...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Assemble the complete VoiceShop specification delivery."
    )
    ...
    manifest_path = output_dir / "VoiceShop-Spec-Complete.manifest.json"
    sums_path = output_dir / "VoiceShop-Spec-Complete.SHA256SUMS.txt"
    report_path = output_dir / "VoiceShop-Spec-Assembly-Report.md"
        ...
        try:
            with zipfile.ZipFile(archive_path, "r") as archive:
                ...
                if bad_member is not None:
                    corrupt.append(f"{archive_path.name}: {bad_member}")
                    continue
                    ...
                    if target.exists():
                        ...
                        if existing_hash == digest:
                            ...
                            conflicts.append(
                                ...
                                f"incoming={digest}, part={number:02d})"
                            )
                            entries.append(
                                Entry(
                                    number,
    ...
    final_hash = sha256_file(final_zip)
    with sums_path.open("a", encoding="utf-8") as stream:
        stream.write(f"{final_hash}  {final_zip.name}\n")
